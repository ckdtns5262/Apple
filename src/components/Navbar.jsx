...
const Navbar = () => {
  ...
  return (
    <div>
      <div className={`${modal === true ? 'bg-[#161617] transition-all ease-out duration-1000' : 'bg-[#434344]'} p-2 text-white font-semibold relative z-0`}>
        <div className="flex items-center justify-between">
          <DiApple className="text-[28px] ml-9" onClick={()=>navigate("/")} />
          {navbars.map((name, i) => (
            <a key={i} className="text-[12px] hidden lg:flex" href={`/${name}`}
              onMouseOver={()=>{{setModal(true); setCName(name)}}}
